	...
	if (this.id=="player"){
		if (this.tween_agite_heart_flag) {
			this.tween_agite_heart_flag=false
				this.tween_agite_heart_opponent=game.add.tween(menuPaper_opponent.scale).to({x:this.scale.x+2,y:this.scale.y+2},100,Phaser.Easing.Linear.None,true,0)
				this.tween_agite_heart=game.add.tween(this.scale).to({x:this.scale.x+2,y:this.scale.y+2},100,Phaser.Easing.Linear.None,true,0)
...
Menu.prototype.modif_heart_at_top = function() {
	if (this.id=="opponent"){
		this.amount_of_heart_paper.text=parameter.number_heart_opponent
			console.log(little_roll_opponent)
